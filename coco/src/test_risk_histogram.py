...
def plot_histograms(df,gamma,delta):
    ...
    plt.savefig(f'../outputs/histograms/test_{gamma}_{delta}_recall_histogram.pdf')

    sizes = torch.cat(df['size'].tolist(),dim=0)
    sns.displot(data=sizes)
    plt.xlabel('size')
    plt.savefig(f'../outputs/histograms/test_{gamma}_{delta}_size_histogram.pdf')

def experiment(gamma,delta,num_lam,num_calib,epsilon,num_trials):
    fname = f'../.cache/test_{gamma}_{delta}_dataframe.pkl'
    ...
    try:
        ...
        labels2 = torch.zeros_like(labels)
        ...
        df.to_pickle(f'../.cache/test_{gamma}_{delta}_dataframe.pkl')

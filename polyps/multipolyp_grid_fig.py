...
def get_grid(gamma, delta, num_plot, num_calib, num_lam, deltas_precomputed, num_grid_hbb, ub, ub_sigma, epsilon, maxiters, output_dir):
    ...
    lambda_hat_adaptive = get_lambda_hat_clt_perpolyp_01(calib_regions, calib_masks, gamma, delta, num_lam)#get_lambda_hat_hbb_perpolyp_01(calib_regions, calib_masks, gamma, delta, num_lam, tlambda)
    nc_list = [1,2]

    list_img_list, list_result_list = get_results(lambda_hat_adaptive, nc_list, val_img_names, val_regions, val_masks, val_num_components, num_plot)
